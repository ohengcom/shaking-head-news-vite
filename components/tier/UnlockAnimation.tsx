...
export function UnlockAnimation({ show, onComplete, message, className }: UnlockAnimationProps) {
  ...
  const resolvedMessage = message || t('unlockSuccess')
    ...
    >
      <div className="animate-in fade-in zoom-in-95 flex flex-col items-center gap-4 duration-500">
        ...
        <div className="text-center">
          <p className="text-foreground text-xl font-semibold">{resolvedMessage}</p>
          <p className="text-muted-foreground mt-1 text-sm">{t('unlockDescription')}</p>
        </div>
      </div>
    </div>
  )
